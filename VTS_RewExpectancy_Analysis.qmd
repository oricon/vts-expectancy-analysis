...
```
1 excluded with 184 subjects total
## ACC data cleaning`
...
excludeSubjs <- unique(excludeSubjs)
```
Exclude 6 ACC outliers out of 184
...
```{r}
summary_corrRT <- cleanRTdata %>% 
  group_by(participant, blockNum) %>% 
  summarise(meanRT = mean(RT, na.rm = TRUE), .groups="keep")
ggplot(data = summary_corrRT, aes(x=blockNum, y=meanRT)) + geom_line(stat = "summary_bin", fun = median) +
  scale_y_continuous(breaks = seq(550,700,25)) + theme_minimal()
```
...
```
``` {r} 
summary_corrACC <- cleanACCdata %>% 
  group_by(participant, blockNum) %>% 
  summarise(ACC = mean(acc, na.rm = TRUE), .groups="keep")
ggplot(data = summary_corrACC, aes(x=blockNum, y=ACC)) + geom_line(stat = "summary_bin", fun = median) +
  ylim(.8,1)
```
...
```
```{r}
afex_plot(switch_m1, x = "prevRew", trace = "rewCond", data_plot = F)

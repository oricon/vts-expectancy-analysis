...
demos <- rbind(survey_data_inlabDet, survey_data_inlabProb, survey_data_online)
demos <- survey_data %>% distinct(ID, .keep_all = TRUE)
only_surveys <- demos$ID[!(demos$ID %in% fulldata$participant)]
...
summary_RT %>% 
  ggplot(aes(x=meanRT)) + 
  #facet_grid(rows = vars(rewExpectancy)) +
  ...
  geom_vline(data = filter(summary_RT, rewExpectancy=="low"),
             aes(xintercept=median(meanRT)), color="red", linetype="dashed", size=1) +
...
plot_model(choice_m5, type = "emm", terms = c("prevRew", "rewCond", "rewExpectancy"), 
           legend.title = "Reward Prospect", colors = "Dark2", 
           axis.title = c("Previous Trial Reward Outcome", "Switch Probability"), 
           title = "", line.size=1)
